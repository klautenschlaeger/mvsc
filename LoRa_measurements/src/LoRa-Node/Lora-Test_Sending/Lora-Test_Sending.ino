...
void send_multipoly(){
  ...
  if (loads<5) {
    ...
    int e=0;
    for (int a=1;a<=2;a++) { //loads
      int part_id = polygon_id*100 + loads*10 + a;
      memcpy(&message[0], &id, 4);
      memcpy(&message[4], &part_id, 4);

      int c = 2;
      for(int i=0;i<30;i++){ //62
        double element = packet[i];
        memcpy(&message[4*c], &element, 8);
        c = c + 2;
        }
      clock_t begin = clock();
      LoRa.beginPacket();
  ...
  Serial.println("1,"+String(count)+","+String(polygon_id)+","+String(sum_bytes)+","+String(time_total,5));
}
